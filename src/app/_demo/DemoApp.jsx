...
function SignalDetail({ s, t, amount, go }) {
  if (!s) return (
    <div>
      ...
      <Card t={t}><div style={{ color: t.muted, textAlign: "center", padding: 24, fontWeight: 600 }}>לא נבחר פריט</div></Card>
    </div>
  );
  const sim = simulate(amount, s.entryPrice, s.currentPrice);
...
export default function DemoApp({ initialView = "dashboard", embedded = false }) {
  ...
  useEffect(() => {
    setView(initialView);
    if (initialView === "signal") setSelSignal((s) => s || SIGNALS[0]);
    if (initialView === "person") setSelPerson((p) => p || PEOPLE[0]);
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: t.bg, color: t.text, fontFamily: "'Heebo', 'Assistant', -apple-system, system-ui, sans-serif" }}>
      ...
      {!embedded && (<header style={{ position: "sticky", top: 0, zIndex: 20, background: t.bg + "ee", backdropFilter: "blur(10px)", borderBottom: `1px solid ${t.border}` }}>
      ...
      {!embedded && (<nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 20, background: t.surface + "f5", backdropFilter: "blur(10px)", borderTop: `1px solid ${t.border}`, display: "flex", overflowX: "auto", padding: "6px 8px", gap: 2 }}>
